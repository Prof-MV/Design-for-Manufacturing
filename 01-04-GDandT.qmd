...
::: {.callout-think}
**Before You Read:** Traditional plus-minus tolerancing creates a square
tolerance zone. Why might a circular tolerance zone be more appropriate for
locating a hole? Think about how a bolt fits into a hole.
:::
...
Traditional dimensioning (Figure \@fig-traditional-vs-gdt) can lead to:
...
::: {.callout-industry}
**Industry Application:** In aerospace and automotive manufacturing, GD&T is
mandatory. Boeing, Airbus, Ford, and GM all require GD&T on engineering
drawings. A single misinterpreted tolerance could result in parts that don't
fit, leading to costly rework or safety issues.
:::
```{r fig-traditional-vs-gdt, echo=FALSE, fig.cap="Comparison: Traditional vs. GD&T Dimensioning"}
...
p1 <- ggplot() +
  ...
  annotate("text", x=2, y=2.5, label="Feature", fontface="bold", size = 5 * diagram_text_scale) +

  annotate("text", x=2, y=1.5, label="25.0 ± 0.5 mm", size = 4 * diagram_text_scale) +

  annotate("text", x=2, y=0.5, label="Traditional: Square Tolerance Zone", 

           fontface="italic", size = 3 * diagram_text_scale) +
...
p2 <- ggplot() +
  ...
  annotate("text", x=2, y=1.5, label="Ø0.5 mm", size = 4 * diagram_text_scale) +
...
#### The Rule of Interchangeability
...
for (i in 1:5) {
  ...
  grid.text(fcf_labels[i], x=fcf_x[i], y=0.5, gp=gpar(fontsize=9 * diagram_text_scale, fontface="bold"))
}

## Add annotations
grid.text("Feature Control Frame Components",
          x=0.5, y=0.95, gp=gpar(fontsize=14 * diagram_text_scale, fontface="bold"))
...
kable(modifiers_df, caption=if (knitr::is_latex_output()) "Common GD\\&T Modifiers and Symbols" else "Common GD&T Modifiers and Symbols", booktabs=TRUE)
```
...
ggplot() +
  ...
  annotate("text", x=51.5, y=50.5, label="Datum Intersection", size=3 * diagram_text_scale, color="red") +
  ...
  annotate("text", x=50, y=49.3, label="Ø0.7 mm\\nTolerance Zone", 
           ...
           size=3 * diagram_text_scale, color="blue", fontface="bold") +
...
### Datum Systems {#sec-datum-systems}
...
### Material Conditions and Bonus Tolerances {#sec-material-conditions}
...
ggplot(hole_data, aes(x=reorder(condition, diameter), y=diameter, fill=condition)) +
  geom_col(width=0.6) +
  geom_text(aes(label=diameter), vjust=-0.5, size=5 * diagram_text_scale, fontface="bold") +
  scale_fill_manual(values=c("MMC"="darkgreen", "Nominal"="gray", "LMC"="darkred")) +
  coord_cartesian(ylim = c(9.5, 10.5)) +
  theme_minimal() +
  labs(
...
ggplot(bonus_df, aes(x=actual_size, y=positional_tolerance)) +
  ...
  annotate("text", x=9.9, y=0.35, label="Base Tolerance = 0.4 mm", size=4 * diagram_text_scale) +
  annotate("text", x=10.1, y=0.65, label="Bonus Tolerance = Actual - MMC", size=3 * diagram_text_scale, angle=45) +
  theme_minimal() +
  labs(
  ...
  coord_cartesian(xlim = c(9.7, 10.3), ylim = c(0.2, 0.85))
```

### Common GD&T Applications {#sec-gdt-applications}
...
ggplot() +
  ...
  annotate("text", x=5, y=10, label="10", size=3 * diagram_text_scale) +

  annotate("text", x=25, y=5, label="25", size=3 * diagram_text_scale, color="red", fontface="bold") +
...
ggplot(data, aes(x=x, y=y, color=type, linewidth=type)) +
  ...
  annotate("text", x=0.5, y=6, label="Datum A\\n(Shaft Axis)", size=3 * diagram_text_scale, color="blue") +
...
### Tolerance Stack-Up Analysis {#sec-stack-up}
...
ggplot(results_df, aes(x=reorder(Condition, Total_Length), y=Total_Length, fill=Condition)) +
  geom_col(width=0.6) +
  geom_text(aes(label=paste(Total_Length, "mm")), vjust=-0.5, size=5 * diagram_text_scale, fontface="bold") +
...
#### Statistical Stack-Up (Optional Advanced Topic) {-}
...
### Classroom Activity: Interactive Tolerance Design {-}
