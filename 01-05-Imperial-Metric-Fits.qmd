...
- Identify and select appropriate clearance, transition, and interference fits
- Use ANSI B4.1 tables to determine imperial fit specifications
- Apply the ISO metric tolerance system with letter designations
- Select preferred metric fits for common engineering applications

::: {.callout-think}
**Before You Read:** Think about a door hinge. The pin must rotate freely in
the knuckles. Now think about a gear pressed onto a motor shaft. It must never
slip. How do engineers specify these very different requirements?
:::

In precision manufacturing, the relationship between two mating parts—typically
a hole and a shaft—is defined by the "fit." Understanding fits is crucial for
ensuring that mechanical assemblies function correctly, whether they need to
slide freely or be permanently locked together.
...
### Imperial Fits {#sec-imperial-fits}
...
Force or shrink fits constitute a special type of interference fit, normally characterized by maintenance of constant bore pressures throughout the range of sizes. As shown in the mathematical section of Chapter @sec-imperial-fits, the interference varies almost directly with diameter.
...
::: {.callout-industry}
**Industry Application:** In automotive transmission manufacturing, bearings
require precise interference fits to prevent spinning on the shaft while
allowing the bearing to rotate freely. Companies like Ford and GM specify fits
to the micrometer level, and incorrect fits can lead to premature bearing
failure and warranty claims.
:::

### Key Takeaways {-}

- Fits define the relationship between mating parts (hole and shaft)
- Hole basis systems are more common because holes are easier to standardize
- Clearance fits allow movement; interference fits create permanent joints
- Transition fits may result in either slight clearance or interference
- The metric ISO system uses letters (uppercase for holes, lowercase for shafts)
  combined with IT grades to specify tolerances
- Always select from preferred fits when possible to reduce manufacturing costs

::: {.callout-check}
**Quick Check:** For a bearing that needs to rotate freely on a shaft, would
you specify a clearance fit or an interference fit? What class of fit (RC, LC,
FN, etc.) might be appropriate?
:::
