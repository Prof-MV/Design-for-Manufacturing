...
```{r torque-angle-plot, echo=TRUE, message=FALSE, warning=FALSE, fig.width=7, fig.height=5, fig.cap="Torque-Angle Tightening Curve"}
library(ggplot2)
library(dplyr)

# Create synthetic data that matches the general shape
set.seed(10)

tibble(
  angle = seq(0, 100, length.out = 400),
  torque_base = 8.5 * (1 - exp(-angle / 18)) +   # sigmoid-like rise
    0.7 * sin(angle/18 * 2*pi) * exp(-angle/60),  # small dip
  extra = pmax(0, (angle - 65)/35)^1.9 * 3.2,          # late sharp rise
  torque = pmin(torque_base + extra, 9.8)              # hard cap
) -> dat

# Limit line value (for annotation)
limit_y <- 9.8

ggplot(dat, aes(angle, torque)) +

  # Main curve
  geom_line(linewidth = 1.1, color = "#1e88e5") +

  # Limit line
  geom_hline(yintercept = limit_y,
             color = "red", linewidth = 1.1, linetype = "solid") +

  # Arrows / annotations
  annotate("segment",
           x = 68, xend = 68, y = limit_y - 0.4, yend = limit_y + 0.4,
           color = "red", linewidth = 1.05, arrow = arrow(length = unit(3.5, "mm"))) +

  annotate("segment",
           x = 68, xend = 92, y = limit_y + 0.15, yend = limit_y + 0.15,
           color = "red", linewidth = 1.05,
           arrow = arrow(length = unit(3.5, "mm"), type = "closed")) +

  # Text labels
  annotate("text", x = 38, y = limit_y + 0.55,
           label = "Limit", color = "red", size = 4.6, hjust = 0, fontface = "bold") +

  annotate("text", x = 80, y = limit_y + 0.55,
           label = "Additional\nAngle", color = "red", size = 4, hjust = 0.5) +

  annotate("text", x = 92, y = max(dat$torque) + 0.25,
           label = "→", color = "#1e88e5", size = 7, hjust = 0.5, vjust = 0.4) +
...
* **Highest Accuracy:** Preload accuracy of ±1–5% is achievable, as friction is completely eliminated from the measurement.
...
* **Reusable Bolts:** Since the bolt is not over-stressed, it can be reused with confidence in its remaining life.
