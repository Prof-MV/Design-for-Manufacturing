...
## Strength of Bolt Joint Calculations {#sec-chap-03-bolts}
...
### Learning Objectives {#sec-objectives-03}

By the end of this chapter, you will be able to:

1. **Calculate** bolt preload and the relationship between tightening torque and tension
2. **Determine** fastener stiffness ($k_b$) and member stiffness ($k_m$) for bolted joints
3. **Apply** the joint stiffness constant to analyze load distribution
4. **Evaluate** safety factors for bolt joint integrity (yield, load, and separation factors)
5. **Design** bolted connections that prevent joint separation and fatigue failure

::: {.callout-think}
**Before You Read:** Why do mechanics use torque wrenches when tightening critical bolts, rather than just tightening "until it feels tight"? What could go wrong with an improperly tightened bolt?
:::

### Mechanics of Bolted Joint Integrity {#sec-bolted-joints}
...
```{r fig-bolt-plot, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Relationship between Bolt Force and External Load."}
library(ggplot2)
library(rlang)
library(vctrs)
...
As illustrated in Figure ~@fig-bolt-plot, the bolt force increases much more slowly while the joint is clamped ($C < 1$). Once the external load exceeds the separation point, the bolt must carry the entire load, significantly increasing the risk of fatigue or tensile failure.

::: {.content-visible when-format="html"}
*This one is live — edit the preload (`preload`) and joint stiffness constant (`C`) below and re-run the cell to see how the separation point and bolt force move.*

```{webr-r}
#| label: bolt-load-response-interactive
## Edit these two values and re-run!
preload <- 5000   ## bolt preload F_i (N)
C       <- 0.25   ## joint stiffness constant C = k_b / (k_b + k_m)

## Separation point: external load at which member clamp force reaches zero
sep_point <- preload / (1 - C)

## Bolt force at a few sample external loads
sample_P <- c(0, 2000, 4000, 6000, 8000, 10000)
bolt_force <- ifelse(sample_P < sep_point,
                     preload + C * sample_P,   ## joint still clamped
                     sample_P)                 ## joint separated - bolt carries all

cat("Joint stiffness constant C =", C, "\n")
cat("Preload F_i               =", preload, "N\n")
cat("Joint separates at P      =", round(sep_point, 1), "N\n\n")

cat(sprintf("%12s  %14s  %s\n", "Ext. load P", "Bolt force F_b", "State"))
for (i in seq_along(sample_P)) {
  state <- if (sample_P[i] < sep_point) "clamped" else "SEPARATED"
  cat(sprintf("%10.0f N  %12.0f N  %s\n", sample_P[i], bolt_force[i], state))
}

cat("\nWhile clamped, each 1000 N of external load adds only",
    round(C * 1000, 0), "N to the bolt.\n")
cat("After separation, each 1000 N of external load adds a full 1000 N to the bolt.\n")
```
:::
...
### Bolt Definitions {#sec-bolt-definitions}
...
```{r fig-basic-profile, echo=FALSE, out.width="70%", fig.align="center", fig.cap="Basic Thread Profile"}
knitr::include_graphics("images/BasicProfile.png")
```

> **Note:** The dimensions and geometry in the Figure ~@fig-basic-profile are only used for 3d modeling and drawing the threads, they are not dimensions to manufacture to!
...
```{r tbl-english-cross-section-area-table, echo=FALSE, message=FALSE, warning=FALSE}
library(knitr)
library(dplyr)
library(kableExtra)
...
```{r tbl-english-washer-dimensions-table, echo=FALSE, message=FALSE, warning=FALSE}
library(knitr)
library(dplyr)
library(kableExtra)
...
### Bolt Tightening Methods {#bolt-tightening-methods}

Achieving the correct preload in a bolted joint is critical for joint integrity. Three primary methods are used in industry to control bolt tension during assembly: torque-controlled tightening, torque-angle tightening, and direct elongation measurement. Each method offers different levels of accuracy and is suited to different applications.

#### Why Tighten the Nut, Not the Bolt?

In most bolted joint assemblies, it is preferable to **rotate the nut while holding the bolt stationary** rather than turning the bolt head. This practice offers several important advantages:

1. **Reduced Torsional Stress in the Bolt:** When the bolt is rotated during tightening, the threads experience friction that induces torsional stress in addition to the desired tensile stress. This combined loading reduces the effective preload and can lead to premature fatigue failure. Rotating the nut instead transfers the frictional torque to the nut-to-washer interface, keeping the bolt in pure tension.

2. **Consistent Friction Conditions:** The friction coefficient under the bolt head is typically more consistent when the bolt remains stationary. The bearing surface under a rotating bolt head can gall or score, leading to unpredictable torque-tension relationships.

3. **Prevention of Bolt Rotation Damage:** In many assemblies, the bolt passes through a clearance hole in the top member. If the bolt rotates during tightening, it may damage the hole surface or any protective coatings. Holding the bolt stationary prevents this damage.

4. **Accessibility:** In structural applications, the nut side is often more accessible for wrench placement, making it the natural choice for the rotating element.

> **Note:** There are exceptions to this rule. In some applications where the nut is captured or inaccessible, the bolt must be rotated. In these cases, anti-seize compounds or specialized lubricants help reduce the torsional stress effects.

---

#### Method 1: Torque-Controlled Tightening (Torque Wrench) {#torque-wrench-method}

The most common method for achieving bolt preload is **torque-controlled tightening** using a torque wrench. This method relies on the relationship between applied torque and resulting bolt tension.

##### How It Works {-}

A torque wrench measures the rotational force applied to the nut. When the specified torque value is reached, the wrench signals the operator (via a click, breakover, or digital display) to stop tightening. The torque is related to preload by the short-form torque equation:

$$T = K \cdot d \cdot F_i$$

Where $T$ is the applied torque, $K$ is the nut factor (typically 0.15–0.25), $d$ is the nominal bolt diameter, and $F_i$ is the resulting preload.

##### Advantages {-}

* **Simplicity:** Torque wrenches are widely available, easy to use, and require minimal training.
* **Speed:** Tightening is fast compared to more precise methods.
* **Cost-Effective:** Equipment costs are low, and the method is suitable for high-volume production.
* **Traceability:** Torque values can be documented for quality records.

##### Disadvantages {-}

* **High Variability:** Approximately 70–90% of the applied torque is consumed by friction in the threads and under the nut face. Small changes in lubrication, surface finish, or thread condition can cause preload variations of ±25–35%.
* **No Direct Tension Measurement:** The actual bolt tension is not measured; it is inferred from the torque-tension relationship.
* **Sensitive to Lubrication:** Accidental lubrication can dramatically increase preload, potentially causing bolt failure (as demonstrated in Example 2, Question 8).

---

#### Method 2: Torque-Angle Tightening {#torque-angle-method}

The **torque-angle method** (also called "turn-of-nut" method) improves upon pure torque control by adding a controlled angular rotation after reaching an initial "snug" torque. This method is widely used in automotive and structural steel applications.

##### How It Works {-}

1. **Snug Tightening:** The bolt is first tightened to a low "snug" torque that brings all joint surfaces into firm contact. This establishes a consistent starting point.
2. **Angle Turn:** From the snug position, the nut is rotated through a specified additional angle (e.g., 60°, 90°, or 120°). This rotation directly corresponds to bolt elongation.

The relationship between rotation angle and elongation is:

$$\delta = \frac{\theta}{360°} \cdot p$$

Where $\delta$ is the bolt elongation, $\theta$ is the rotation angle in degrees, and $p$ is the thread pitch.

```{r fig-torque-angle-plot, echo=FALSE, message=FALSE, warning=FALSE, fig.width=7, fig.height=5, fig.cap="Torque-Angle Tightening Curve"}
library(ggplot2)

# 1. Generate synthetic data for the curve
# We want an initial rise, a plateau, and then a sharp upward turn.
x_vals <- seq(0, 10, length.out = 200)
y_vals <- 0.1 * x_vals^3 - 1.2 * x_vals^2 + 4.5 * x_vals # Polynomial to mimic the shape
y_vals <- y_vals / max(y_vals) * 5 # Scale for aesthetics

df <- data.frame(Angle = x_vals, Torque = y_vals)

# Define specific points for the red annotations
limit_x <- 7.8
limit_y <- df$Torque[which.min(abs(df$Angle - limit_x))]
extra_x <- 9.2
extra_y <- df$Torque[which.min(abs(df$Angle - extra_x))]

# 2. Build the plot
ggplot(df, aes(x = Angle, y = Torque)) +
  # The main curve
  geom_line(color = "#3b5998", size = 1.5) +
  
  # Red limit lines (Horizontal and Vertical)
  annotate("segment", x = 0, xend = limit_x, y = limit_y, yend = limit_y, color = "red") +
  annotate("segment", x = limit_x, xend = limit_x, y = 0, yend = limit_y, color = "red") +
  
  # Additional angle vertical line
  annotate("segment", x = extra_x, xend = extra_x, y = 0, yend = extra_y, color = "red") +
  
  # Red arrow following the curve
  annotate("segment", x = 8.2, xend = 9, y = 3.2, yend = 4.2, 
           color = "red", size = 1.2, arrow = arrow(length = unit(0.3, "cm"))) +
  
  # Labels
  annotate("text", x = -0.2, y = limit_y, label = "Limit", hjust = 1, size = 5 * diagram_text_scale) +
  annotate("text", x = limit_x, y = -0.3, label = "Additional\nAngle", vjust = 1, size = 4.5 * diagram_text_scale) +
  
  # Axis Arrows and styling
  theme_classic() +
  ...
  labs(x = "Angle", y = "Torque")
```

As shown in Figure @fig-torque-angle-plot, the tightening process begins with a rapid torque rise as surfaces come into contact. Once the "snug" limit is reached, the additional angle rotation stretches the bolt into the elastic (or slightly plastic) region, achieving consistent preload regardless of friction variations.

##### Advantages {-}

* **Reduced Friction Sensitivity:** Since the angle turn directly controls elongation, friction variations after the snug point have less effect on final preload.
* **Higher Preload Accuracy:** Typical preload scatter is reduced to ±10–15%, compared to ±25–35% for torque-only methods.
* **Detects Anomalies:** If the torque rises too quickly or slowly during the angle turn, it indicates problems such as cross-threading, galling, or incorrect bolt grade.

##### Disadvantages {-}

* **Requires Calibration:** The snug torque and angle values must be determined experimentally for each joint configuration.
* **More Complex Tooling:** Requires angle-measuring capability (electronic torque-angle wrenches or manual protractor fixtures).
* **Not Suitable for All Joints:** Joints with soft gaskets or highly compliant members may not respond predictably to angle control.

---

#### Method 3: Elongation Measurement {#elongation-method}

The most accurate method for controlling bolt preload is **direct measurement of bolt elongation**. Since bolt tension is directly proportional to elongation through Hooke's law, measuring elongation eliminates the uncertainties associated with friction.

##### How It Works {-}

The bolt is tightened while its elongation is monitored using one of several techniques:

1. **Mechanical Measurement:** Using a micrometer or dial indicator to measure the overall bolt length before and after tightening. This requires access to both ends of the bolt.

2. **Ultrasonic Measurement:** An ultrasonic transducer is placed on the bolt head, and the time-of-flight of an ultrasonic pulse traveling through the bolt is measured. As the bolt stretches, the path length increases, and the travel time changes proportionally. Modern ultrasonic bolt gauges can measure elongation to ±0.0001 inches (2.5 μm).

3. **Load-Indicating Washers:** Special washers with protrusions that flatten under load. The gap between the washer and the joint surface indicates the achieved preload.

The relationship between elongation and preload is:

$$F_i = k_b \cdot \delta = \frac{A_t \cdot E}{L} \cdot \delta$$

Where $F_i$ is the preload, $k_b$ is the bolt stiffness, $\delta$ is the elongation, $A_t$ is the tensile stress area, $E$ is the modulus of elasticity, and $L$ is the grip length.

4. **SmartBolts:** are specialized fasteners with a built-in Visual Indication System™ that changes color from red to black to accurately indicate when proper tension (torque) is achieved. As the bolt stretches during tightening, the indicator in the head darkens, providing real-time, precise, and permanent monitoring of joint security without extra tools. 

```{r video-SmartBolts, echo=FALSE, results='asis'}
embed_youtube("ClPwlpa86bY", "SmartBolts changing color as they are tightened")
```


Key Details About Color-Changing Bolts:

* How it Works: The fastener incorporates a, patented mechanism within the bolt head. It appears red when loose and gradually turns black as the bolt stretches to the correct tension.

* Accuracy: Unlike relying solely on torque wrenches, which can be affected by friction, dirt, or rust, these bolts measure actual clamp force (tension/elongation), providing a more reliable, real-time indication of tightness.

* Applications: They are primarily used in heavy machinery, manufacturing, and critical joints to easily identify if a bolt is loose without manual inspection.

* Customization: SmartBolts can be customized to change color at specific design tensions ranging from 30-90% of the fastener proof strength.

These fasteners are designed to be a permanent, visual verification tool for maintenance and safety. 

##### Advantages {-}

* **Highest Accuracy:** Preload accuracy of ±1–5% is achievable with measurements, as friction is completely eliminated from the measurement.
* **Direct Measurement:** The actual bolt tension is measured, not inferred from torque.
* **Ideal for Critical Applications:** Essential for pressure vessels, nuclear reactors, aerospace structures, and other safety-critical joints.

##### Disadvantages {-}

* **High Cost:** Ultrasonic equipment is expensive, and the method requires trained operators.
* **Time-Consuming:** Each bolt must be measured individually, making the method slow for large-volume production.
* **Access Requirements:** Mechanical methods require access to both ends of the bolt; ultrasonic methods require a clean, flat bolt head surface.
* **Calibration Required:** Ultrasonic measurements must be calibrated for each bolt material and geometry.

---

#### Comparison of Tightening Methods

| Method | Preload Accuracy | Speed | Cost | Best Application |
| :--- | :---: | :---: | :---: | :--- |
| Torque Wrench | ±25–35% | Fast | Low | General assembly, non-critical joints |
| Torque-Angle | ±10–15% | Moderate | Medium | Automotive, structural steel |
| Elongation Measurement | ±1–5% | Slow | High | Pressure vessels, aerospace, nuclear |

> **Design Tip:** For critical applications, specify the tightening method on engineering drawings. Include the target preload, acceptable tolerance, and any special lubrication requirements.
...
For a visual representation, consider the force triangle plot shown in Figure @fig-force-triangle-plot, which illustrates the vector addition of forces in a simplified bolted joint scenario.
...
```{r fig-force-triangle-plot, echo=FALSE, out.width="70%", fig.align="center", fig.cap="Force Triangle"}
...
::: {.content-visible when-format="html"}
*This one is live — edit the preload, the bolt/member stiffnesses, and the external working load below, then re-run the cell to see how the load is shared.*

```{webr-r}
#| label: joint-working-load-interactive
## Edit these values and re-run!
preload    <- 30000   ## bolt preload F_i (N)
kb         <- 431027  ## bolt stiffness k_b (N/mm)
km         <- kb * 4  ## member stiffness k_m (N/mm) - stiffer than the bolt here
external_p <- 15000   ## external working load P on the joint (N)

## Load sharing
Phi             <- kb / (kb + km)          ## joint stiffness constant C
bolt_increase   <- Phi * external_p        ## extra tension taken by the bolt
member_decrease <- (1 - Phi) * external_p  ## clamp force given up by the members

## Resulting forces
Fb <- preload + bolt_increase              ## total bolt tension
Fm <- (1 - Phi) * external_p - preload     ## member force (negative = still clamped)

cat("Joint stiffness constant C = k_b / (k_b + k_m) =", round(Phi, 3), "\n\n")
cat("External working load P        =", external_p, "N\n")
cat("  -> carried by the bolt       =", round(bolt_increase, 0), "N  (",
    round(100 * Phi, 1), "% of P )\n")
cat("  -> relieved from the members =", round(member_decrease, 0), "N  (",
    round(100 * (1 - Phi), 1), "% of P )\n\n")
cat("Preload F_i        =", preload, "N\n")
cat("Total bolt load F_b =", round(Fb, 0), "N\n")
cat("Member force F_m    =", round(Fm, 0), "N\n\n")

if (Fm < 0) {
  cat("F_m < 0: the members are still in compression - joint stays CLAMPED.\n")
  cat("Separation would occur at P =", round(preload / (1 - Phi), 0), "N\n")
} else {
  cat("F_m >= 0: the members have SEPARATED - the bolt now carries the full load.\n")
}
```
:::
...
### Advanced Analysis of Bolted Joints with applicable Formulas {#sec-bolted-analysis-formulas}
...
* $E$: Modulus of elasticity based on this table:


```{r elastic-modulus-table, echo=FALSE, message=FALSE, warning=FALSE}
library(knitr)
library(dplyr)
library(kableExtra)

## Creating the material property data frame
modulus_lookup <- tibble::tribble(
  ~Material,         ~Metric_GPa, ~Imperial_Mpsi,
  "Steel (Low Carbon)",    200,       29,
  "Steel (Stainless 304)", 193,       28,
  "Iron (Ductile)",        165,       24,
  "Iron (Gray)",           110,       13,
  "Copper (Pure)",         117,       17,
  "Brass (Yellow)",        105,       15.2,
  "Bronze (Phosphor)",     110,       16,
  "Aluminum (6061)",       69,        10,
  "Titanium (Ti-6Al-4V)",  114,       16.5,
  "Magnesium Alloy",       45,        6.5
)

## Prepare headers with line breaks and units
modulus_cols <- c(
    "Material", 
    "Elastic Modulus<br>($E$) GPa", 
    "Elastic Modulus<br>($E$) $10^6$ psi"
)

## Render the table
modulus_lookup %>%
  kbl( 
      caption = "Elastic Modulus of Common Engineering Metals",
      booktabs = TRUE,
      digits = 1,
      align = "lcc",
      escape = FALSE, 
      col.names = modulus_cols
      ) %>%
  kable_styling(latex_options = c("hold_position"), full_width = FALSE) %>%
  ## Highlighting the Metric and Imperial data columns differently
  column_spec(2, background = "#E7F0F9") %>% 
  column_spec(3, background = "#F9E7F0") %>%
  add_header_above(c(" " = 1, "Metric" = 1, "Imperial" = 1))
```
...
1.  **Tensile Stress Area ($A_t$):** From Table @tbl-english-cross-section-area-table for a "1/2" size UNC bolt.
2.  **Washer Thickness ($t_w$):** From Table @tbl-english-washer-dimensions-table for a "1/2 N" washer.

##### Step 2: Calculation {-}

::: {.content-visible when-format="html"}
*This one is live — every design parameter and table lookup is a variable at the top of the cell. Change the load, the plate thicknesses, the moduli, the bolt size, or the looked-up areas and re-run to recompute the stiffnesses, the joint constant, and all three safety factors.*

```{webr-r}
#| label: bolt-safety-analysis-interactive
## ---- 1. Design parameters (edit these) --------------------------------------
P    <- 4000     ## external tensile load on the joint (lbf)
Eb   <- 29e6     ## modulus of elasticity, bolt - carbon steel (psi)
E1   <- 30e6     ## modulus of elasticity, plate 1 - steel (psi)
E2   <- 14.5e6   ## modulus of elasticity, plate 2 - cast iron (psi)
Sp   <- 85000    ## proof strength, SAE Grade 5 (psi)
t1   <- 1.25     ## thickness of steel plate (in)
t2   <- 0.75     ## thickness of cast iron plate (in)

## ---- 2. Values looked up from the standards tables (edit these) -------------
At   <- 0.1419   ## tensile stress area, 1/2 in-13 UNC  (Table @tbl-english-cross-section-area-table)
tw   <- 0.095    ## washer thickness, 1/2 N            (Table @tbl-english-washer-dimensions-table)
d_nom <- 0.5     ## nominal bolt diameter (in)
L_total <- 3.0   ## selected bolt length (in)
Hex_W <- 0.75    ## hex-head width across flats for a 1/2 in bolt (in)
clr   <- 0.0625  ## hole clearance (in)

## ---- 3. Geometry ----------------------------------------------------------
Grip <- t1 + t2 + tw                 ## total grip length l
Lt   <- 2 * d_nom + 0.25             ## thread length (standard formula for L <= 6 in)
l_d_total <- L_total - Lt            ## total unthreaded shank length of the bolt
l_d  <- min(l_d_total, Grip)         ## unthreaded portion inside the grip
l_t  <- Grip - l_d                   ## threaded portion inside the grip
Ad   <- (pi / 4) * d_nom^2           ## unthreaded (shank) cross-section area

## ---- 4. Stiffnesses -----------------------------------------------------------
## Bolt stiffness k_b (shank + threaded portion acting as springs in series)
kb <- (Ad * At * Eb) / (Ad * l_t + At * l_d)

## Member stiffness k_m via a substitute cylinder (large-joint form)
Dh <- d_nom + clr                                   ## effective hole diameter
As <- (pi / 4) * ((Hex_W + 0.1 * Grip)^2 - Dh^2)    ## substitute-cylinder area
E_avg <- (E1 * t1 + E2 * t2) / (t1 + t2)            ## thickness-weighted average modulus
km <- (E_avg * As) / Grip

## ---- 5. Joint constant and forces ------------------------------------------
C_joint <- kb / (kb + km)            ## joint stiffness constant C
Fi <- 0.75 * At * Sp                 ## preload at 75% of proof load
Fb <- C_joint * P + Fi               ## total bolt load

## ---- 6. Safety factors ------------------------------------------------------
nP <- (Sp * At) / Fb                 ## yield (proof) safety factor
nL <- (Sp * At - Fi) / (C_joint * P) ## load factor
n0 <- Fi / (P * (1 - C_joint))       ## separation factor

## ---- 7. Report -----------------------------------------------------------------
verdict <- function(n) if (n > 1) "SAFE" else "FAIL"

cat(sprintf("Grip length l           = %8.3f in\n", Grip))
cat(sprintf("Thread length L_T       = %8.3f in\n", Lt))
cat(sprintf("Unthreaded shank l_d    = %8.3f in  (in grip: %.3f in)\n", l_d_total, l_d))
cat(sprintf("Shank area A_d          = %8.4f in^2\n", Ad))
cat(sprintf("Bolt stiffness k_b      = %8.0f lbf/in\n", kb))
cat(sprintf("Substitute area A_s     = %8.4f in^2\n", As))
cat(sprintf("Member stiffness k_m    = %8.0f lbf/in\n", km))
cat(sprintf("Joint constant C        = %8.4f\n", C_joint))
cat(sprintf("Preload F_i             = %8.0f lbf\n", Fi))
cat(sprintf("Total bolt load F_b     = %8.0f lbf\n\n", Fb))

cat(sprintf("Yield safety   n_P = %6.2f  -> %s\n", nP, verdict(nP)))
cat(sprintf("Load factor    n_L = %6.2f  -> %s\n", nL, verdict(nL)))
cat(sprintf("Separation     n_0 = %6.2f  -> %s\n", n0, verdict(n0)))
```
:::
...
::: {.content-visible when-format="html"}
**Live calculator for Example 2**

*Every worked answer below (Questions 1--8) is reproduced in this one cell. Edit any input at the top — the bolt geometry, the plate materials and thicknesses, the total bolted-up load, the number of bolts, the friction factor `K` — and re-run to watch $k_b$, $k_m$, $C$, the forces, the stresses, the three safety factors, and the tightening torque all update. Try Question 8's scenario by setting `K <- 0.10` (lubricated) and see the bolt overstress.*

```{webr-r}
#| label: example2-interactive
## ---- Inputs (edit these and re-run) ---------------------------------------
d_nom  <- 0.5      ## nominal bolt diameter (in)
L_bolt <- 1.5      ## bolt length under the head (in)
At     <- 0.1599   ## tensile stress area, 1/2 in-20 UNF (in^2)
Eb     <- 29e6     ## bolt modulus of elasticity (psi)
Sp     <- 85000    ## proof strength, SAE J429 Grade 5 (psi)
Su     <- 120000   ## min tensile strength, Grade 5 (psi)

t_top  <- 0.5      ## steel top plate thickness (in)
t_bot  <- 0.75     ## cast iron bottom plate thickness (in)
tw     <- 0.095    ## washer thickness under the head (in)
E_top  <- 29e6     ## steel plate modulus (psi)
E_bot  <- 13e6     ## cast iron plate modulus (psi)

Dh     <- 0.510    ## effective hole diameter (in)
W_head <- 1.5 * d_nom   ## head width across flats ~ 1.5 d  (in)

P_total <- 10000   ## total external load carried by the joint (lbf)
n_bolts <- 4       ## number of bolts sharing the load
K       <- 0.20    ## torque / nut factor (0.20 dry; ~0.10 lubricated)

## ---- Q1: bolt stiffness k_b ---------------------------------------------------
Lt   <- 2 * d_nom + 0.25          ## thread length, L <= 6 in
l_d  <- L_bolt - Lt               ## unthreaded shank length
lj   <- t_top + t_bot + tw        ## total joint (grip) length
l_t  <- lj - l_d                  ## threaded length within the grip
Ad   <- (pi / 4) * d_nom^2        ## major-diameter (shank) area
kb   <- (Ad * At * Eb) / (Ad * l_t + At * l_d)

cat("Q1  l_d =", round(l_d, 3), "in   l_t =", round(l_t, 3),
    "in   A_d =", round(Ad, 4), "in^2\n")
cat("Q1  k_b =", format(signif(kb, 3), big.mark = " "), "lbf/in\n\n")

## ---- Q2: member stiffness k_m ----------------------------------------------
As   <- (pi / 4) * ((W_head + 0.1 * lj)^2 - Dh^2)   ## substitute-cylinder area
k_steel <- (E_top * As) / (t_top + tw)              ## steel + washer sub-cylinder
k_iron  <- (E_bot * As) / t_bot                     ## cast iron sub-cylinder
km   <- 1 / (1 / k_steel + 1 / k_iron)              ## springs in series

cat("Q2  A_s =", round(As, 4), "in^2\n")
cat("Q2  k_steel =", format(signif(k_steel, 3), big.mark = " "), "lbf/in",
    "   k_iron =", format(signif(k_iron, 3), big.mark = " "), "lbf/in\n")
cat("Q2  k_m =", format(signif(km, 3), big.mark = " "), "lbf/in\n\n")

## ---- Q3: joint constant C, load share P, preload F_i ---------------------
C   <- kb / (kb + km)
P   <- P_total / n_bolts
Fi  <- 0.75 * At * Sp

cat("Q3  C =", round(C, 3), "   P per bolt =", round(P, 0), "lbf",
    "   F_i =", round(Fi, 0), "lbf\n\n")

## ---- Q4 / Q5: bolt force F_b and member force F_m -----------------------------
Fb  <- C * P + Fi
Fm  <- (1 - C) * P - Fi

cat("Q4  F_b = C*P + F_i =", round(Fb, 0), "lbf\n")
cat("Q5  F_m = (1-C)*P - F_i =", round(Fm, 0), "lbf",
    if (Fm < 0) " (negative -> members still in compression)\n" else "\n")
cat("\n")

## ---- Q6: tensile stress in the bolt -----------------------------------------
sigma_b <- Fb / At
cat("Q6  sigma_b = F_b / A_t =", round(sigma_b, 0), "psi  (",
    round(sigma_b / 1000, 1), "ksi )\n\n")

## ---- Q7: safety factors ---------------------------------------------------
nP <- (Sp * At) / (C * P + Fi)     ## proof / yield
nL <- (Sp * At - Fi) / (C * P)     ## load factor
nO <- Fi / (P * (1 - C))           ## separation

cat("Q7  n_P =", round(nP, 2), "   n_L =", round(nL, 2),
    "   n_O =", round(nO, 2), "\n\n")

## ---- Q8: tightening torque and the effect of lubrication -----------------
Tin <- K * d_nom * Fi             ## torque to reach F_i (in-lbf)
cat("Q8  T =", round(Tin, 0), "in-lbf =", round(Tin / 12, 0), "ft-lbf",
    "  (at K =", K, ")\n")

## If the operator applies that same torque to a lubricated bolt (K ~ 0.10),
## the preload that actually develops is much higher:
K_lube <- 0.10
Fi_lube <- Tin / (K_lube * d_nom)
sigma_lube <- Fi_lube / At
cat("Q8  Same torque on a lubricated bolt (K =", K_lube, "): F_i =",
    round(Fi_lube, 0), "lbf  ->  sigma =", round(sigma_lube, 0), "psi\n")
cat("Q8  Tensile strength S_u =", Su, "psi  ->",
    if (sigma_lube > Su) "STRESS EXCEEDS S_u: the bolt breaks while tightening.\n"
    else "still below S_u.\n")
```
:::
