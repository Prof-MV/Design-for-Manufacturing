...
Table \@ref(tab:lower-pair-summary-10) summarises the key characteristics of each lower-pair joint.

```{r lower-pair-summary-10, echo=FALSE, message=FALSE}
lower_pairs <- tribble(
...
::: {.callout-discussion}
**Group Discussion:** Look at the universal (Hooke/Cardan) joint shown in Figure \@ref(fig:fig-compound-joint-table-10). It has connectivity 2. Where do you see this joint used in an automobile drivetrain, and why is the 2-DOF flexibility important there?
:::
...
::: {.callout-activity}
**Try This:** Look at the recliner chair or excavator in Figure \@ref(fig:fig-practice-links-10) below. Sketch the kinematic schematic and classify each link as binary, ternary, or quaternary before checking the chapter practice section.
:::
...
- **RRRR** — four-bar linkage (four revolute joints) → Figure \@ref(fig:fig-linkage-numbering-10), left
- **RRRP** — slider-crank (three revolute + one prismatic) → Figure \@ref(fig:fig-linkage-numbering-10), right
...
As Figure \@ref(fig:fig-dof-body-10) shows, a single rigid body moving freely in a plane has **3 DOF**:
...
ggplot() +
  ...
  annotate("text", x = 2.5, y = 4.9, label = "n  =  number of links", fontface = "bold", size = 4, color = "#1565C0") +
  annotate("text", x = 2.5, y = 4.4, label = "Include ground link (link 1)", size = 3.2) +
  annotate("text", x = 2.5, y = 3.9, label = "Each free link = 3 DOF", size = 3.2, color = "steelblue") +
  geom_rect(aes(xmin = 4.3, xmax = 7.5, ymin = 3.5, ymax = 5.2), fill = "#FFF8E1", color = "#F57F17", linewidth = 1.2) +
  annotate("text", x = 5.9, y = 4.9, label = "j\u2081  =  full joints", fontface = "bold", size = 4, color = "#F57F17") +
  annotate("text", x = 5.9, y = 4.4, label = "Revolute (R), Prismatic (P)", size = 3.2) +
  annotate("text", x = 5.9, y = 3.9, label = "Each removes 2 DOF", size = 3.2, color = "#E65100") +
  geom_rect(aes(xmin = 7.8, xmax = 11, ymin = 3.5, ymax = 5.2), fill = "#F3E5F5", color = "#7B1FA2", linewidth = 1.2) +
  annotate("text", x = 9.4, y = 4.9, label = "j\u2082  =  half joints", fontface = "bold", size = 4, color = "#7B1FA2") +
  annotate("text", x = 9.4, y = 4.4, label = "Cam pair, Gear pair", size = 3.2) +
  annotate("text", x = 9.4, y = 3.9, label = "Each removes 1 DOF", size = 3.2, color = "#4A148C") +
  geom_rect(aes(xmin = 1, xmax = 11, ymin = 1.8, ymax = 3.2), fill = "#E8F5E9", color = "#2E7D32", linewidth = 1) +
  annotate("text", x = 6, y = 2.9, label = "Compound Joint Rule", fontface = "bold", size = 4, color = "#2E7D32") +
  annotate("text", x = 6, y = 2.45,
           label = "If m links share one joint location  \u2192  count it as (m \u2212 1) full joints", size = 3.5) +
  annotate("text", x = 6, y = 2.0,
           label = "Example: 3 links at one pin = 2 full joints  (j\u2081 += 2, not 1)", size = 3.2, color = "gray30") +
...
Determine the mobility of the planar four-bar linkage shown in Figure \@ref(fig:fig-ex1-fourbar-10). All joints are revolute (pin) joints.
:::
...
**Step 4: Apply Equation** \@ref(eq:gruebler-10)
...
Determine the mobility of the planar linkage shown in Figure \@ref(fig:fig-ex2-complex-10). The mechanism is planar and **all joints have connectivity 1**.
:::
...
Determine the mobility of the planar linkage in Figure \@ref(fig:fig-ex3-compound-10). All joints have connectivity 1. **Links 3, 4, and 5 are all connected at the same revolute joint axis** (compound joint).
:::
...
Determine the mobility of the planar linkage shown in Figure \@ref(fig:fig-ex4-higher-pair-10). The mechanism has 11 links and is planar. **One joint is a pin-in-slot** (labelled $f_i = 2$, meaning it permits 2 DOF, i.e. it is a half joint). All other joints are full revolute joints.
:::
...
Counting from Figure \@ref(fig:fig-ex4-higher-pair-10): links 1 through 11.
...
ggplot(mob_data, aes(x = factor(M), y = 1)) +
  ...
  geom_text(aes(label = label), size = 3.2, lineheight = 1.3, fontface = "bold") +
  geom_text(aes(x = "1", y = 1.45, label = "TARGET"), color = "#2E7D32", fontface = "bold", size = 4.5) +
...
Setting $M = 1$ and $j_2 = 0$ in Equation \@ref(eq:gruebler-10) and solving gives the relationship $j_1 = \frac{3n - 4}{2}$, which requires $n$ to be even. Table \@ref(tab:m1-solutions-10) lists valid combinations.
...
```{r m1-solutions-10, echo=FALSE, message=FALSE}
m1_sols <- tribble(
...
From the schematic in Figure \@ref(fig:fig-frontend-loader-schematic-10):
...
b) Calculate the mobility using Equation \@ref(eq:gruebler-10)
...
An excavator arm (see Figure \@ref(fig:fig-practice-links-10)) has the following schematic: ground link + 4 moving links, 5 revolute joints, and 1 compound joint where 3 links share the same pin axis.
