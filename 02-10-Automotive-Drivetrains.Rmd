...
In a **rear-wheel drive** (RWD) vehicle, the engine is typically at the front, but power is delivered to the rear wheels through a propeller shaft and rear differential. Figure \@ref(fig:fwd-rwd-diagram-13) shows how the transmission output (item 3) connects to the rear axle (4) via the propshaft (5).
...
- Power is *continuously* distributed to all four wheels through a centre differential (item 9 in Figure \@ref(fig:awd-4wd-diagram-13))
...
- The transfer case (item 6 in Figure \@ref(fig:awd-4wd-diagram-13)) mechanically locks the front and rear propshafts together — both axles are forced to rotate at the same speed
...
```{r ev-single-motor-13, echo=FALSE, out.width="55%", fig.align="center", fig.cap="Single-motor EV drivetrain. A single electric motor drives one axle (front or rear) through a fixed-ratio reduction gear and a conventional open differential. This is the simplest and lowest-cost EV powertrain configuration."}
knitr::include_graphics("images/drTrain-image10.png")
```

As shown in Figure \@ref(fig:ev-single-motor-13), the simplest EV drivetrain uses one motor to drive a single axle. The motor output passes through a fixed single-ratio reduction gear and then a conventional differential to the two driven wheels.
...
```{r inhub-motor-photo-13, echo=FALSE, out.width="60%", fig.align="center", fig.cap="In-hub electric motor by NTN, installed directly inside the wheel assembly. The motor, reduction gearing, and brake disc are all packaged within the wheel rim, completely eliminating axle shafts, CV joints, and differentials."}
knitr::include_graphics("images/drTrain-image14.jpg")
```

```{r inhub-motor-exploded-13, echo=FALSE, out.width="65%", fig.align="center", fig.cap="Exploded view of an in-hub motor assembly, showing the rotor, stator, power electronics, bearing, capacitor ring, protective cover, and integrated brake disc and caliper — the complete drivetrain for one wheel, packaged entirely within the wheel rim."}
knitr::include_graphics("images/drTrain-image16.png")
```

The most radical EV drivetrain concept places the motor **directly inside each wheel hub**. As Figures \@ref(fig:inhub-motor-photo-13) and \@ref(fig:inhub-motor-exploded-13) show, the entire motor is contained within the wheel assembly — the vehicle underbody is completely free of axles, differentials, and driveshafts.
