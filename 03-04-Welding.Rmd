...
Figure \@ref(fig:basic-weld-terms-ref) provides a professional reference showing these key terms in context for both fillet and groove weld configurations.

```{r basic-weld-terms-ref, echo=FALSE, fig.cap="Basic welding terms showing fillet weld and groove weld anatomy with key dimensions labeled. Adapted from Jensen (2016).", fig.align='center', out.width='90%'}
knitr::include_graphics("images/C26_F01_pg210.jpg")
```

::: {.callout-check}
**Quick Check:** Looking at Figure \@ref(fig:basic-weld-terms-ref), can you identify the difference between the "weld size (leg)" on a fillet weld versus the "throat" dimension? Why is the throat dimension critical for weld strength calculations?
:::
...
```{r basic-joint-types-ref, echo=FALSE, fig.cap="The five basic welding joint configurations: Butt, Lap, Corner, Tee, and Edge joints. Adapted from Jensen (2016).", fig.align='center', out.width='95%'}
knitr::include_graphics("images/C26_F02_pg211.jpg")
```

Figure \@ref(fig:basic-joint-types-ref) shows these five fundamental joint configurations as 3D representations. Each joint type has specific applications based on the geometry of the parts being joined and the loading conditions.
...
ggplot() +
  ...
  ## Throat formula
  annotate("text", x = 8.2, y = 2.8, label = "t = 0.707 × L", color = "red", size = 4, fontface = "bold") +
...
Figure \@ref(fig:weld-types-config-ref) provides a comprehensive reference showing single and double configurations for various weld types.

```{r weld-types-config-ref, echo=FALSE, fig.cap="Types of welds showing single and double configurations for fillet, square groove, bevel groove, V-groove, J-groove, and U-groove welds. Adapted from Jensen (2016).", fig.align='center', out.width='95%'}
...
Figure \@ref(fig:weld-symbol-elements-ref) shows the standard location of elements in a welding symbol according to AWS (American Welding Society) standards.

```{r weld-symbol-elements-ref, echo=FALSE, fig.cap="Standard location of elements of a welding symbol showing all possible positions for size, length, pitch, groove angle, and supplementary symbols. Adapted from Jensen (2016).", fig.align='center', out.width='85%'}
knitr::include_graphics("images/C26_F03_pg211.jpg")
```

**Key points from Figure \@ref(fig:weld-symbol-elements-ref):**
...
Figure \@ref(fig:basic-weld-symbols-ref) shows the basic weld symbols for common weld types, organized by category.

```{r basic-weld-symbols-ref, echo=FALSE, fig.cap="Basic weld symbols shown on the reference line, including fillet, plug/slot, spot, seam, and various groove weld symbols. Adapted from Jensen (2016).", fig.align='center', out.width='95%'}
...
Figure \@ref(fig:supplementary-symbols-ref) provides the standard AWS supplementary symbols as a quick reference.

```{r supplementary-symbols-ref, echo=FALSE, fig.cap="Supplementary welding symbols including weld all around, field weld, melt-thru, backing/spacer material, consumable insert, and contour symbols (flush, convex, concave). Adapted from Jensen (2016).", fig.align='center', out.width='95%'}
...
When a specific welding process is required, it is indicated in the tail of the welding symbol using standard letter designations. Figure \@ref(fig:process-designations-ref) provides the complete AWS designation system.

```{r process-designations-ref, echo=FALSE, fig.cap="Designation of welding processes by letters according to AWS standards. Includes brazing, oxyfuel gas welding, resistance welding, arc welding, and other specialized processes. Adapted from Jensen (2016).", fig.align='center', out.width='70%'}
...
Figure \@ref(fig:typical-fillet-welds-ref) provides a comprehensive reference for interpreting fillet weld symbols with various dimension configurations.

```{r typical-fillet-welds-ref, echo=FALSE, fig.cap="Typical fillet weld symbol configurations showing: (1) arrow side, (2) other side, (3) both sides with different sizes, (4) weld all around, (5) contour symbol, (6) postweld finishing, (7-9) intermittent weld patterns. Adapted from Jensen (2016).", fig.align='center', out.width='60%'}
knitr::include_graphics("images/C26_F10_pg214.jpg")
```

**Key interpretations from Figure \@ref(fig:typical-fillet-welds-ref):**
...
::: {.callout-check}
**Quick Check:** Looking at row 9 in Figure \@ref(fig:typical-fillet-welds-ref), why might an engineer specify staggered intermittent welds rather than aligned intermittent welds?
:::
...
**Example from Figure \@ref(fig:groove-combined-dims-ref):**
...
```{r pjp-welds-ref, echo=FALSE, fig.cap="Single-groove welds with partial joint penetration (PJP). The dimension in parentheses indicates the depth of groove preparation, while the dimension without parentheses indicates the effective throat (weld size). Adapted from Jensen (2016).", fig.align='center', out.width='65%'}
...
**Reading PJP symbols from Figure \@ref(fig:pjp-welds-ref):**
...
Figure \@ref(fig:fillet-weld-assignment-ref) shows a comprehensive fillet weld assignment with multiple weld configurations.

```{r fillet-weld-assignment-ref, echo=FALSE, fig.cap="Fillet weld assignment (A-66) showing five different assemblies with various fillet weld specifications. Practice interpreting weld symbols and creating drawing callouts. Adapted from Jensen (2016).", fig.align='center', out.width='75%'}
...
::: {.callout-activity}
**Try This:** For Assembly D in Figure \@ref(fig:fillet-weld-assignment-ref), identify:
...
Figure \@ref(fig:shaft-support-assignment-ref) presents a more complex welded assembly with multiple components and weld specifications.

```{r shaft-support-assignment-ref, echo=FALSE, fig.cap="Shaft support welding assignment (A-67) showing a multi-component weldment with various fillet weld specifications for ribs, arms, shafts, and base connections. Adapted from Jensen (2016).", fig.align='center', out.width='75%'}
...
Figure \@ref(fig:groove-welds-assignment-ref) presents three assemblies focusing on groove weld specifications.

```{r groove-welds-assignment-ref, echo=FALSE, fig.cap="Groove welds assignment (A-69). Three assemblies with various groove weld types including bevel, V-groove, and backing welds. The circled areas indicate locations where detailed weld sketches should be made. Adapted from Jensen (2016).", fig.align='center', out.width='75%'}
