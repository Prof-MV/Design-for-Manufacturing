...
The following animation demonstrates screw/bolt force calculations:
...
```{r joint-working-load, echo=TRUE, fig.cap="Joint Diagram with External Working Load"}
library(ggplot2)
...
## Advanced Analysis of Bolted Joints {#bolted-analysis}
...
### Member Stiffness ($k_m$) and Substitute Cylinders
...
```{r joint-analysis, echo=TRUE}
# Input Variables
E <- 210000        # Modulus (MPa)
At <- 84.3         # Tensile Area M12 (mm^2)
Ad <- 113.1        # Shank Area M12 (mm^2)
lt <- 10           # Threaded length in grip (mm)
ld <- 40           # Unthreaded length (mm)
P <- 15000         # External Load (N)
Sp <- 600          # Proof Strength (MPa)

# 1. Calculate Bolt Stiffness
kb <- (Ad * At * E) / (Ad * lt + At * ld)

# 2. Assume Member Stiffness (Simplified for example)
km <- kb * 4 

# 3. Calculate Constants and Safety
C <- kb / (kb + km)
Fi <- 0.75 * At * Sp
no <- Fi / (P * (1 - C)) # Separation Factor

print(paste("Joint Constant C:", round(C, 3)))
print(paste("Separation Safety Factor (no):", round(no, 2)))
```
